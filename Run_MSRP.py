...
from utilities.tools import review_to_words,get_embedding
from utilities.tools import extract_features,load_model
from utilities.classify_ensemble import predict_MSRP_test_data
...
train_nlp_features = np.load("handcrafted_features/msrp_train.npy")
test_nlp_features =  np.load("handcrafted_features/msrp_test.npy")
...
train.columns=["Quality","id1","id2","question1","question2"]
test.columns=["Quality","id1","id2","question1","question2"]
...
test_labels=np.array(test["Quality"])

print("using all the trained models and predict the test data")
predict_MSRP_test_data(n_folds,nb_words,test_nlp_features,test_data_1,test_data_2,test_labels)
